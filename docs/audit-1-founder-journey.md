# FrontDeskOS Audit 1 — Founder Journey Audit
...
FrontDeskOS has the bones of a working platform — the AI receptionist is genuinely impressive, the booking engine works, and the operations dashboard gives owners real utility. But the founder experience of onboarding a paying client is fragile, manual, and missing critical tooling.
...
1. Founder navigates to `https://frontdeskos.vercel.app/login`
...
### Friction Points
- New founders will not know `https://frontdeskos.vercel.app/login` exists. There is no public landing page at `/` that explains what FrontDeskOS is or provides a login link. The root `/` redirects to `/dashboard-placeholder` which is a generic page.
...
- **No engagement metrics.** The founder cannot answer: "Is Client A actually using FrontDeskOS?" without logging into the client's admin dashboard (which they cannot do without the client's credentials).
...
2. Navigate to `https://frontdeskos.vercel.app/{slug}`
...
4. Hope the owner receives the reset email (which might go to spam because it's from Supabase, not from "FrontDeskOS")
